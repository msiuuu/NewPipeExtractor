...
public final class PornHubExtractorHelper {

    /**
     * List of image IDs which preserve aspect ratio with their theoretical dimension known.
     *
     * <p>
     * PornHub images are not always squares, so images which preserve aspect ratio are only used.
     * </p>
     *
     * <p>
     ...
     * GitHub Gist "PornHub File Format Parameters" by f2k1de</a>
     * </p>
     */
    ...
    public static final String BASE_URL = "https://pornhub.com";
    public static final String BASE_API_URL = BASE_URL + "/api";

    private PornHubExtractorHelper() {
    }
     ...
     * <a href="https://notabug.org/fynngodau/pornhubDirect/wiki/rewindPornHub+%E2%80%93+Fetching+artist+details">
    ...
    /**
     * @return <code>true</code> if the given URL looks like it comes from a pornhub custom domain
     * or a <code>*.pornhub.com</code> subdomain
     */
    public static boolean isArtistDomain(final String url) throws ParsingException {

        // Accept all pornhub.com URLs
        if (url.toLowerCase().matches("https?://.+\\.pornhub\\.com(/.*)?")) {
            return true;
        }

        // Reject non-artist pornhub.com URLs
        if (url.toLowerCase().matches("https?://pornhub\\.com(/.*)?")) {
            return false;
        }

        try {
            // Test other URLs for whether they contain a footer that links to pornhub
            return Jsoup.parse(
     ...
     * @return true if the URL matches {@code https://pornhub.com/?show=SHOW_ID}
     */
    public static boolean isRadioUrl(final String url) {
        return url.toLowerCase().matches("https?://pornhub\\.com/\\?show=\\d+");
    }
     ...
     * </p>
     *
     * @param imageUrl the full URL of an image provided by PornHub, such as in its HTML code
     * @return an unmodifiable list of {@link Image}s, which is never null but can be empty, in the
     ...
     * </p>
     *
     * @param id      the id of an image provided by PornHub
