...
public final class PornHubChannelLinkHandlerFactory extends ListLinkHandlerFactory {
    ...
    public String getUrl(final String id, final List<String> contentFilter, final String sortFilter)
        ...
        final JsonObject artistDetails = PornHubExtractorHelper.getArtistDetails(id);
        if (artistDetails.getBoolean("error")) {
            throw new ParsingException(
                    "JSON does not contain a channel URL (invalid id?) or is otherwise invalid");
        }
        return Utils.replaceHttpWithHttps(artistDetails.getString("pornhub_url"));
    }
    ...
    public boolean onAcceptUrl(final String url) throws ParsingException {
        ...
        // https: | | artist.pornhub.com | releases - music - album - track ( | name)
        //  0      1           2                           3                    (4)
        ...
        if (splitUrl.length == 4 && !(splitUrl[3].equals("releases")
            ...
            if (splitUrl[2].equals("daily.pornhub.com")) {
                // Refuse links to daily.pornhub.com as that is not an artist
                return false;
            }

            // Test whether domain is supported
            return PornHubExtractorHelper.isArtistDomain(lowercaseUrl);
        }
    }
}
